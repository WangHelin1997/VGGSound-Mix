...
def fix_length(s1, s2, s3, s4, tag1, tag2, tag3, tag4, fixed_len=5, sr=24000):
    # tag: start time
    # Fix length
    s1_out, s2_out, s3_out, s4_out = np.zeros(int(sr*fixed_len)), np.zeros(int(sr*fixed_len)), np.zeros(int(sr*fixed_len)), np.zeros(int(sr*fixed_len))
...
def create_mixes(speaker_num, s1_samples, s2_samples, s3_samples, s4_samples):
    if speaker_num == 0:
        mix = np.zeros_like(s1_samples)
    elif speaker_num == 1:
        mix = s1_samples
    elif speaker_num == 2:
        mix = s1_samples + s2_samples
    elif speaker_num == 3:
        mix = s1_samples + s2_samples + s3_samples
    elif speaker_num == 4:
        mix = s1_samples + s2_samples + s3_samples + s4_samples
    return mix
